...
const arrayBufferToBase64 = arrbuf => {
  ...
  return btoa(
    u8bit.reduce((data, byte) => data + String.fromCharCode(byte), "")
  )
}
  ...
  assembleCardServiceUrls = () => {
    ...
    return sanityClient.observable.fetch('*[_id == "siteSettings"][0]').pipe(
      map(siteConfig => {
        const doc = { ...displayed }
        if (siteConfig.logo) {
          const siteLogoImageUrl = urlFor(siteConfig.logo).width(500).url()
  ...
  fetchData = async () => {
    ...
    this.fetchSubscription = of(null)
      .pipe(
        ...
        map(
          arrayBuffer =>
            `data:image/${fileType};base64,${arrayBufferToBase64(arrayBuffer)}`
        )
      )
      .subscribe({
