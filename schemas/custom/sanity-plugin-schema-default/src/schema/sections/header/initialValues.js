import {
  defaultMainImage,
  defaultImageArray,
  defaultButton,
  defaultLinks,
  defaultForm,
} from "../../common/defaults";

export const variantAInitialValue = {
  heading: "Build & Launch without problems",
  description:
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque efficitur nisl sodales egestas lobortis.",
  primaryButton: defaultButton("Get Started"),
  secondaryButton: defaultButton("How it works"),
  mainImage: defaultMainImage(
    "image-169ed7b2d1aefee7f232cb11d591a87afb081800-1050x700-jpg"
  ),
};

export const variantBInitialValue = {
  heading: "Build & Launch without problems",
  description:
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque efficitur nisl sodales egestas lobortis.",
  primaryButton: defaultButton("Get Started"),
  secondaryButton: defaultButton("How it works"),
  images: defaultImageArray([
    "image-0b68b4267d6d520230fe4b107e622b99a6739678-941x734-jpg",
    "image-6149e873e52c7b945a489b9bb65204e3905bed81-1050x701-jpg",
    "image-85ba15e7e2930aa280c5d7e46dae17a018caf3a7-967x725-jpg",
    "image-8ab26c1d233fa8ce762fe3ccb4c37df46139edac-1190x669-jpg",
  ]),
};

export const variantCInitialValue = {
  heading: "Learn how We will help you to reach your goals",
  primaryButton: defaultButton("Get Started"),
  secondaryButton: defaultButton("How it works"),
  youtubeLink: "https://www.youtube.com/watch?v=videoId",
};

export const variantDInitialValue = {
  heading: "Build & Launch without problems",
  description:
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque efficitur nisl sodales egestas lobortis.",
  primaryButton: defaultButton("Get Started"),
  secondaryButton: defaultButton("How it works"),
  mainImage: defaultMainImage(
    "image-e7eec379f0a0f57b3d2215dba9e28ced4e2bc420-1050x700-jpg"
  ),
};

export const variantEInitialValue = {
  heading: "Build & Launch without problems",
  plainText:
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque efficitur nisl sodales egestas lobortis.",
  primaryButton: defaultButton("Get Started"),
  secondaryButton: defaultButton("How it works"),
  form: defaultForm("Header - Variant E", [
    {
      name: "First Name",
      type: "inputText",
    },
    {
      name: "Last Name",
      type: "inputText",
    },
    {
      name: "name@email.com",
      type: "inputEmail",
    },
    {
      name: "Enter your password",
      type: "inputPassword",
    },
  ]),
  formLinks: defaultLinks(["Policy Privacy", "Terms of Use"]),
};
