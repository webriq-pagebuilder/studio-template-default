...
import AuthorsComponent from "../../reserved_categories";
...
function EditCategory({ category }) {
  ...
  const { del } = useDocumentOperation(publishId, "category");
    ...
    try {
      setTimeout(async function () {
        ...
        if (res.length === 0) {
          ...
          toast.push({
            status: "error",
            title: "The category you are trying to delete is used to a page.",
          });
        }
  ...
  return (
    <Container width={10}>
      {open && (
        <Dialog
          header={`Are you sure you want to delete this author: ${name}?`}
          id="delete-page"
          onClose={onClose}
          zOffset={1000}
        >
      ...
      >
        ...
        >
          ...
          <Text
            size={2}
            style={{ color: "lightgray", marginRight: 7, cursor: "pointer" }}
            onClick={() => handleRoute("Categories", null)}
          >
            Categories
          </Text>
          <Text
