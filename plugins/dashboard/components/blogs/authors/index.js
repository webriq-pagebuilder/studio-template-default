...
import { Grid, Tooltip, Text, Box, Button, Container, Card } from "@sanity/ui";
// import styled from "styled-components";
import DashboardComponent from "../../dashboard";
import BlogsComponent from "..";
import { AiOutlineUserAdd, AiOutlineEye } from "react-icons/ai";
import AddAuthor from "./iframe/addAuthor";
import { urlFor } from "../../../utils/imageBuilder";
import EditAuthor from "./iframe/editAuthor";
// import DashboardComponent from "../dashboard";
...
function AuthorsComponent() {
  ...
  async function getAuthors() {
    await client
      .fetch('*[_type == "author"] | order(_updatedAt desc)')
  ...
  } else if (component === "AddAuthor") {
    return <AddAuthor />;
  } else if (component === "EditAuthor") {
  ...
  return (
    <Container width={10} style={{ background: "#EEEFF3" }} padding={4}>
      <Box style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          ...
          <Text
            size={2}
            style={{ color: "lightgray", marginRight: 7, cursor: "pointer" }}
            onClick={() => handleRoute("Blogs", null, null)}
          >
            Blogs
          </Text>
          ...
          <Text>
            ...
            >
              <Button
                ...
                padding="2"
                tone="primary"
                onClick={() => handleRoute("AddAuthor", null, null)}
      ...
      <Box style={{ display: "flex", padding: 2 }}>
        {authors ? (
          <Grid columns={[2, 3, 4, 5]} gap={[1]} autoRows="auto">
            {authors.map((author) => {
              return (
                <Card
                  padding={2}
                  ...
                  >
                    ...
                    <Box>
                      <Button
                        ...
                        onClick={() =>
                          handleRoute("EditAuthor", author._id, author.name)
                        }
                      />
                    </Box>
                  </Box>
